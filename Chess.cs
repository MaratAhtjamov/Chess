...
public class Chess : MonoBehaviour
{
    ...
    public int ETAP, MIN, X, Y, a, AA, b, M, N, R, J1, x1, y1, x2, y2, POVTOR, MAX, BB, Limit, Limit0, Part, KF, OPAS, PopMat=7;
    public char MyColor;
    ...
    public static bool REVERS, Avto, WhiteH, BlackH, zap, shah, MatFind;
    ...
    void Start()
    {
        ...
        Pos = BeginPosition;
         ...
         // Pos = "rnb4k/ppp3p1/4p3/PP2q3/R7/2P5/2PB4/2K2n2 b - - 1 23";
        //Party.Add( Pos);
    ...
    void Update()
    {
        ...
        if (ETAP == 15)
        {
            ...
            Limit = Limit * MyList.Count / 30;
            dat = DateTime.Now.AddSeconds(Limit); dat0 = DateTime.Now.AddSeconds(Limit / 2);
            Part = 1;
            ...
            else
            {
                ...
                if (hod == "")
                   // if ((BOARD[0, 0] == 'w' && BalansF(Pos) > 400) || (BOARD[0, 0] == 'b' && BalansF(Pos) < -400))                       
                        {
                        //PopMat = UnityEngine.Random.Range(2, 6);
                        if (MatFind == false) PopMat += 1;
                        
                            for (int i = 1; i <= 4; i++)//////////поиск мата
                            {
                                BBB[5, 5].GetComponentInChildren<Text>().text = i + " хода";
                                shah = false;
                                var t1 = DateTime.Now.AddMilliseconds(15500);
                                ///Debug.Log("Ищу мат в " + i + " ход"); TextComment.GetComponent<Text>().text = "Ищу мат в " + i + " ход";
                                var pb = BoardLoad(Pos);
                        var ko= MatBOAR(pb, i, true);///искать кооперативный мат
                        if (ko!="")                            hod = MatBOAR(pb, i, false);
                                if (hod != "")
                                {
                                MatFind = true;
                                ...
                                if (DateTime.Now > t1) { PopMat=0; break; }
                                ////////  if (shah == false ) break;
                            }
                            if (hod == "") MatFind = false;
                        }
        ...
        if (ETAP == 152)
        {
           ...
           /////////////////////////////////////////////////////////     MyList[a] += "   " + LEV[a] + Left(h2, 6);
                ...
                if (CountF(Pos) > 25)///дебют
                {
                    ...
                    if (MyList[a].Contains("f2-f4")) BB -= 25; if (MyList[a].Contains("f7-f5")) BB -= 25;
                        ...
                        if (DateTime.Now > dat0)
                        {
                           ...
                           /// ETAP = 155;
                            ...
                            else
                            {
                                ...
                                if (a > 1 && a < MyList.Count) { ETAP = 155; for (int i = a; i < MyList.Count; i++) BAL[i] = -99999; }
          ...
          if (ETAP == 155)
          {
            Debug.Log("Этап 155 пройден");
             ///  Poradok();
            var s = 0; var n = 0;
              n = MyList.Count;          
            
            
                for (int i = 1; i < n; i++) if (BAL[0]>BAL[i]+5) { MyList[i] = "" + i; BAL[i] = -99999; }
                for (int i = 1; i < n; i++) { MyList.Remove("" + i); BAL.Remove(-99999); }
        ...
        if (ETAP == 100)////редактор ///начальный вход
        {
            ...
            time1 = DateTime.UtcNow.AddMinutes(60) - DateTime.UtcNow;
            time2 = DateTime.UtcNow.AddMinutes(60) - DateTime.UtcNow;
    ...
    { var MATRW = new string[9, 9];
            ...
            for (int y = 1; y <= 8; y++)
                if (MATRB[x, y] != "" || MATRW[x, y] != "")
                {
                    ...
                    /*if (CF > 25)
                    {
                        ...
                        if (x >= 4 && x <= 5 && y >= 4 && y <= 5)
                        {
                            v += MatrSum(MATRW[x, y]) + MatrSum(MATRB[x, y]); v += Math.Sign(F);
                            if (F == 'P' && y == 5) v += 5;
                            if (F == 'p' && y == 4) v -= 5;
                            if (F == 'P' ) v += 5;
                            if (F == 'p' ) v -= 5;
        ...
        if (CF < 10)///эндшпиль
        {
            ...
            {
                if (p.Contains("b") && p.Contains("n") && CF == 4 && (xK == 1 || yK == 1 || xK == 8 || yK == 8)) ///конь и слон
                {
                    if (Mathf.Repeat(xb + yb, 2) == 1)
                    {
                        AAA[1, 1].GetComponentInChildren<Text>().text = "Y";
                        AAA[8, 8].GetComponentInChildren<Text>().text = "Y";
                        if ((xK <= 2 && yK <= 2) || (xK >= 7 && yK >= 7)) v += 50;
                        var centrk1 = Convert.ToInt32(50 * Math.Sqrt((xK - 1) * (xK - 1) + (yK - 1) * (yK - 1)));
                        var centrk2 = Convert.ToInt32(50 * Math.Sqrt((xK - 8) * (xK - 8) + (yK - 8) * (yK - 8)));
                        if (centrk1 < centrk2) centrk = centrk1; else centrk = centrk2;
                    }
                    if (Mathf.Repeat(xb + yb, 2) == 0)
                    {
                        AAA[1, 8].GetComponentInChildren<Text>().text = "Y";
                        AAA[8, 1].GetComponentInChildren<Text>().text = "Y";
                        if ((xK <= 2 && yK >= 7) || (xK >= 7 && yK <= 2)) v += 50;
                        var centrk1 = Convert.ToInt32(50 * Math.Sqrt((xK - 1) * (xK - 1) + (yK - 8) * (yK - 8)));
                        var centrk2 = Convert.ToInt32(50 * Math.Sqrt((xK - 8) * (xK - 8) + (yK - 1) * (yK - 1)));
                        if (centrk1 < centrk2) centrk = centrk1; else centrk = centrk2;
                    }
                }
                v -= centrK;
        ...
        {           
            ...
            if (yk == 8 && xk == 7) v -= 9;
            if (yk == 8 && xk == 3) v -= 9;
           // if ((BOARD[0, 1] != 'K' || BOARD[0, 1] != 'Q') && xK ==5 && yK==1 ) v -= 100;
           // if ((BOARD[0, 2] != 'k' || BOARD[0, 3] != 'q') && xk == 5 && yk == 8) v += 100;
   ...
   /* public List<string> VarPos(string p)
        ...
        if (v == "") ///если больше 2
        {
            ...
            if (Part == 2 && nn>4)
            {
                ...
                var BL = new List<int>();                                    
                    for (int i = 0; i < nn; i++)
                    {
                        var pp1 = HODBOAR(BOAR, Next[i]);
                    if (BOAR[0, 0] == 'w') { BL.Add(BalSil(pp1));  };
                    if (BOAR[0, 0] == 'b') { BL.Add(-BalSil(pp1));  };
                }
                while (1 == 1)
                {
                    var exit = true;
                    ...
                    {
                        if (BL[i - 1] < BL[i])
                        { var swap = BL[i - 1]; BL[i - 1] = BL[i]; BL[i] = swap;
                          var swap2 = Next[i - 1]; Next[i - 1]= Next[i]; Next[i]=swap2;
                          exit = false;
                        }
                    }
                    if (exit) break;
    ...
    public string MatBOAR(char[,] BOAR1, int deep, bool koop)
    {
        var v = "";
        if (deep > 0)
        {
            ...
            for (int i = 0; i < Next.Count; i++)
            {
                ...
                else ///если мат в 1 ход не найден
                {
                    for (int j = 0; j < Next1.Count; j++) ///ответы оппонента
                    {
                        ...
                        var v2 = MatBOAR(pp2, deep - 1, koop);
                        if (v2 == "" && koop == false) { v = "";break; }
                        if (v2 != "" && koop) break;
                    }
                }
                if (v != "") break;
    ...
    public void Tablo()
    {
        ...
        Time1.GetComponent<InputField>().text = time11.ToString("HH:mm:ss") + "";
        if (time1.Minutes < 1)
        {
        ...
        Time2.GetComponent<InputField>().text = time22.ToString("HH:mm:ss") + "";
        if (time2.Minutes < 1)
        {
